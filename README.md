...
- **Automatic Screenshot Capture** - Captures your screen at configurable intervals
...
```bash
pip install flashback-screenshots            # Core only
pip install flashback-screenshots[ocr]       # With OCR
pip install flashback-screenshots[search]    # With search capabilities
pip install flashback-screenshots[webui]     # With web interface
```

### Platform-Specific Dependencies

Flashback can capture active window titles to help you identify screenshots. This requires platform-specific libraries:

**Linux (X11):**
```bash
pip install flashback-screenshots[linux]   # Installs python-xlib
```

**Windows:**
```bash
pip install flashback-screenshots[windows]  # Installs pygetwindow, pywin32
```

**macOS:** Window title capture is not currently supported.

> **Note:** Window title capture is optional. If the platform-specific library is not installed, Flashback will still capture screenshots but won't record which window was active.
